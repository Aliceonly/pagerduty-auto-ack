...
def request_with_retry(method: str, url: str, action: str, **kwargs):
    attempt = 1
    while True:
        try:
            ...
            err_type = "HTTP错误" if isinstance(err, requests.exceptions.HTTPError) else "请求错误"
            print(f"   [RETRY] ATTEMPT {attempt} {err_type}，sleep {wait_seconds}s 后重试: {err}")
...
def delete_all_future_overrides(headers: dict, schedule_id: str, lookahead_days=365):
    ...
    for override in overrides:
        if delete_single_override(headers, schedule_id, override.get('id')):
            ...
            print(f"\n[ERROR] 删除 override {override.get('id')} 失败，停止流程。")
            return False

    total = len(overrides)
    print(f"批量删除完成。成功 {deleted_count}/{total} 个。")
    return True
...
def main():
    ...
    for person_name in schedule_data:
        ...
        if user_id:
            ...
            failed_users.append(person_name)

    if failed_users:
        raise RuntimeError(f"Step2 失败：以下用户未能获取 user id: {failed_users}")
